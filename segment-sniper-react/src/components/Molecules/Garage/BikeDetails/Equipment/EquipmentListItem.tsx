...
const EquipmentListItem = ({ item, setModalState }: EquipmentListItemProps) => {
  ...
  return (
    <Col>
      ...
      <Row>
        <Col className={styles.equipmentLabel}>
          Total Miles / Miles At Install
        </Col>
        <Col>
          {item.totalMiles} / {item.milesAtInstall}
        </Col>
      </Row>
      <Row>
        <Col className={styles.equipmentLabel}>Installed on</Col>
        <Col>
          {timeFormatter.convertStringToFormattedDateTime(
            item.installDate as string,
          )}
        </Col>
      </Row>
